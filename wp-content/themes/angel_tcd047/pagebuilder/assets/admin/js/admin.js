jQuery(document).ready(function($){
 ...
 + '</div>',
	...
	// テキスト概要を表示
	var setWidgetOverview = function(el) {
		var overview = '';
		var $first_text = $(el).find('.pb-modal-edit-widget .pb-content :input').filter(':text, textarea').first();
		if (!$first_text.attr('name').match(/(url|link)/i)) {
			overview = $first_text.val() || '';
		}
		if (overview) {
	...
	$('.pb-rows-container .pb-widget').each(function(){
		setWidgetOverview(this);
	});

	// ウィジェット完了ボタン
	$pb_metabox.on('click.setOverview', '.pb-modal-edit-widget .pb-apply, .pb-modal-edit-widget .pb-close', function(e){
		e.preventDefault();
		e.stopPropagation();
		setWidgetOverview($(this).closest('.pb-widget'));
	});
