...
function wc_review( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-review-<?php comment_ID(); ?>">
		...
		<div class="review-meta reviewmetadata"><a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
			<?php printf( __( '%1$s at %2$s', 'welcart_basic_square' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( __( '(Edit)', 'welcart_basic_square' ), ' ' ); ?>
		</div><!-- .review-meta .reviewmetadata -->
		
		<?php if ( $comment->comment_approved == '0' ) : ?>
			<em><?php _e( 'Thank you for the review. Please wait for a while until it is published.', 'welcart_basic_square' ); ?></em><br />
		<?php else: ?>
