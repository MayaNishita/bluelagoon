...
//				break;
					...
					if( isset($_POST['settlement_selected']) ) {
						...
						foreach( (array)$acting_payments as $payment ) {
							if( !in_array( $payment, $settlement_selected ) ) {
								...
								$mes .= sprintf( __('* %s can not be unselected. Please "delete" or "stop" the payment method.<br />','usces'), $settlement_name );
							}
						}
	...
	function usces_session_start() {
		...
		@session_start();
//usces_p(session_get_cookie_params());
...
?>
	<script type='text/javascript'>
		uscesL10n = {
			...
			'itemOrderAcceptable': "<?php echo $itemOrderAcceptable; ?>",
...
//				$mes .= __('Invalid CANNAT pretend.', 'usces') . "<br />";
	...
	function getItemOrderAcceptable( $post_id ) {
		$str = get_post_meta( $post_id, '_itemOrderAcceptable', true );
		$value = ( !empty($str) ) ? (int)$str : 0;
		return $value;
	}
...
//	}
	...
	function get_pictids($item_code) {
		...
		$ids = apply_filters( 'usces_filter_get_pictids', $results, $item_code );
		return $ids;
	}
...
//usces_log( print_r($res,true), "veritrans.log" );
			...
			$merchant_name = mb_convert_kana( $this->options['company_name'], "RNASKV", 'UTF-8' );
			$banner_url = "";
