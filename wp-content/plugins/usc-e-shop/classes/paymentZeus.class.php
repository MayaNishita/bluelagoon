...
class ZEUS_SETTLEMENT
{
	...
	public function __construct() {
		...
		if( $this->is_activate_card() || $this->is_activate_bank() || $this->is_activate_conv() ) {
			//add_action( 'usces_construct', array( $this, 'acting_construct' ) );
	...
	***********************************************/
	public function initialize_data() {
		...
		if( !in_array( 'zeus', $options['acting_settings'] ) ) {
...
<?php
	...
	public function settlement_update() {
		...
		if( WCUtils::is_blank($mes) ) {
			...
			if( 'on' == $options['acting_settings']['zeus']['bank_activate'] ) {
				...
				$this->payment_structure['acting_zeus_bank'] = '銀行振込決済（ZEUS）';
			} else {
	...
	***********************************************/
	public function settlement_tab_title() {
		...
		$settlement_selected = get_option( 'usces_settlement_selected' );
		if( in_array( 'zeus', (array)$settlement_selected ) ) {
			echo '<li><a href="#uscestabs_zeus">'.__('ZEUS Japanese Settlement','usces').'</a></li>';
		}
	}
...
?>
	...
	<form action="" method="post" name="zeus_form" id="zeus_form">
		<table class="settle_table">
			<tr>
				<th>クレジットカード決済</th>
				<td><input name="card_activate" type="radio" id="card_activate_zeus_1" value="on"<?php if( isset($acting_opts['card_activate']) && $acting_opts['card_activate'] == 'on' ) echo ' checked="checked"'; ?> /></td><td><label for="card_activate_zeus_1">利用する</label></td>
				<td><input name="card_activate" type="radio" id="card_activate_zeus_2" value="off"<?php if( isset($acting_opts['card_activate']) && $acting_opts['card_activate'] == 'off' ) echo ' checked="checked"'; ?> /></td><td><label for="card_activate_zeus_2">利用しない</label></td>
				<td colspan="2"></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_clid_zeus');">カード決済IPコード</a></th>
				<td colspan="4"><input name="clientip" type="text" id="clid_zeus" value="<?php if( isset($acting_opts['clientip']) ) echo esc_html($acting_opts['clientip']); ?>" size="40" /></td>
				<td colspan="2"><div id="ex_clid_zeus" class="explanation">契約時にゼウスから発行されるクレジットカード決済用のIPコード（半角数字）</div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_connection_zeus');">接続方式</a></th>
				<td><input name="connection" type="radio" id="connection_zeus_1" value="1"<?php if( isset($acting_opts['connection']) && $acting_opts['connection'] == 1 ) echo ' checked="checked"'; ?> /></td><td><label for="connection_zeus_1">Secure Link</label></td>
				<td><input name="connection" type="radio" id="connection_zeus_2" value="2"<?php if( isset($acting_opts['connection']) && $acting_opts['connection'] == 2 ) echo ' checked="checked"'; ?> /></td><td><label for="connection_zeus_2">Secure API</label></td>
				<td colspan="2"><div id="ex_connection_zeus" class="explanation">認証接続方法。契約に従って指定する必要があります。</div></td>
			</tr>
			<tr id="authkey_zeus">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_authkey_zeus');">認証キー</a></th>
				<td colspan="4"><input name="authkey" type="text" id="clid_zeus" value="<?php if( isset($acting_opts['authkey']) ) echo esc_html($acting_opts['authkey']); ?>" size="40" /></td>
				<td colspan="2"><div id="ex_authkey_zeus" class="explanation">契約時にゼウスから発行されるSecure API用認証キー（半角数字）</div></td>
			</tr>
			<tr id="3dsecur_zeus">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_3dsecur_zeus');">3Dセキュア（※）</a></th>
				<td><input name="3dsecur" type="radio" id="3dsecur_zeus_1" value="1"<?php if( isset($acting_opts['3dsecur']) && $acting_opts['3dsecur'] == 1 ) echo ' checked="checked"'; ?> /></td><td><label for="3dsecur_zeus_1">利用する</label></td>
				<td><input name="3dsecur" type="radio" id="3dsecur_zeus_2" value="2"<?php if( isset($acting_opts['3dsecur']) && $acting_opts['3dsecur'] == 2 ) echo ' checked="checked"'; ?> /></td><td><label for="3dsecur_zeus_2">利用しない</label></td>
				<td colspan="2"><div id="ex_3dsecur_zeus" class="explanation">3Dセキュアを利用するにはSecure APIを利用した接続が必要です。契約に従って指定する必要があります。</div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_security_zeus');">セキュリティーコード（※）</a></th>
				<td><input name="security" type="radio" id="security_zeus_1" value="1"<?php if( isset($acting_opts['security']) && $acting_opts['security'] == 1 ) echo ' checked="checked"'; ?> /></td><td><label for="security_zeus_1">利用する</label></td>
				<td><input name="security" type="radio" id="security_zeus_2" value="2"<?php if( isset($acting_opts['security']) && $acting_opts['security'] == 2 ) echo ' checked="checked"'; ?> /></td><td><label for="security_zeus_2">利用しない</label></td>
				<td colspan="2"><div id="ex_security_zeus" class="explanation">セキュリティーコードの入力を必須とするかどうかを指定します。契約に従って指定する必要があります。</div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_quickcharge_zeus');">QuickCharge</a></th>
				<td><input name="quickcharge" type="radio" id="quickcharge_zeus_1" value="on"<?php if( isset($acting_opts['quickcharge']) && $acting_opts['quickcharge'] == 'on' ) echo ' checked="checked"'; ?> /></td><td><label for="quickcharge_zeus_1">利用する</label></td>
				<td><input name="quickcharge" type="radio" id="quickcharge_zeus_2" value="off"<?php if( isset($acting_opts['quickcharge']) && $acting_opts['quickcharge'] == 'off' ) echo ' checked="checked"'; ?> /></td><td><label for="quickcharge_zeus_2">利用しない</label></td>
				<td colspan="2"><div id="ex_quickcharge_zeus" class="explanation">ログインして一度購入したメンバーは、次の購入時にはカード番号を入力する必要がなくなります。</div></td>
			</tr>
			<?php if( defined('WCEX_AUTO_DELIVERY') ): ?>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_batch_zeus');">バッチ処理</a></th>
				<td><input name="batch" type="radio" id="batch_zeus_1" value="on"<?php if( isset($acting_opts['batch']) && $acting_opts['batch'] == 'on' ) echo ' checked="checked"'; ?> /></td><td><label for="batch_zeus_1">利用する</label></td>
				<td><input name="batch" type="radio" id="batch_zeus_2" value="off"<?php if( isset($acting_opts['batch']) && $acting_opts['batch'] == 'off' ) echo ' checked="checked"'; ?> /></td><td><label for="batch_zeus_2">利用しない</label></td>
				<td colspan="2"><div id="ex_batch_zeus" class="explanation">ゼウス決済を定期購入でご利用の場合は、「利用する」にしてください。また、QuickCharge も「利用する」にしてください。</div></td>
			</tr>
			<?php endif; ?>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_howpay_zeus');">お客様の支払方法</a></th>
				<td><input name="howpay" type="radio" id="howpay_zeus_1" value="on"<?php if( isset($acting_opts['howpay']) && $acting_opts['howpay'] == 'on' ) echo ' checked="checked"'; ?> /></td><td><label for="howpay_zeus_1">分割払いに対応する</label></td>
				<td><input name="howpay" type="radio" id="howpay_zeus_2" value="off"<?php if( isset($acting_opts['howpay']) && $acting_opts['howpay'] == 'off' ) echo ' checked="checked"'; ?> /></td><td><label for="howpay_zeus_2">一括払いのみ</label></td>
				<td colspan="2"><div id="ex_howpay_zeus" class="explanation">お客様が利用するクレジットカードのカード会社により、選択できる分割回数が異なります。<?php if( defined('WCEX_AUTO_DELIVERY') ) echo '定期購入商品は常に「一括払い」で決済されます。'; ?></div></td>
			</tr>
		</table>
		<table class="settle_table">
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_bank_zeus');">銀行振込決済（入金おまかせサービス）</a></th>
				<td><input name="bank_activate" type="radio" id="bank_activate_zeus_1" value="on"<?php if( isset($acting_opts['bank_activate']) && $acting_opts['bank_activate'] == 'on' ) echo ' checked="checked"'; ?> /></td><td><label for="bank_activate_zeus_1">利用する</label></td>
				<td><input name="bank_activate" type="radio" id="bank_activate_zeus_2" value="off"<?php if( isset($acting_opts['bank_activate']) && $acting_opts['bank_activate'] == 'off' ) echo ' checked="checked"'; ?> /></td><td><label for="bank_activate_zeus_2">利用しない</label></td>
				<td><div id="ex_bank_zeus" class="explanation">銀行振込支払いの自動照会機能です。振込みがあった場合、自動的に入金済みになり、入金確認メールが自動送信されます。</div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_bank_ope_zeus');">稼働環境</a></th>
				<td><input name="bank_ope" type="radio" id="bank_ope_zeus_1" value="test"<?php if( isset($acting_opts['bank_ope']) && $acting_opts['bank_ope'] == 'test' ) echo ' checked="checked"'; ?> /></td><td><label for="bank_ope_zeus_1">テスト環境</label></td>
				<td><input name="bank_ope" type="radio" id="bank_ope_zeus_2" value="public"<?php if( isset($acting_opts['bank_ope']) && $acting_opts['bank_ope'] == 'public' ) echo ' checked="checked"'; ?> /></td><td><label for="bank_ope_zeus_2">本番環境</label></td>
				<td><div id="ex_bank_ope_zeus" class="explanation">動作環境を切り替えます。</div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_bank_clid_zeus');">入金おまかせIPコード</a></th>
				<td colspan="4"><input name="clientip_bank" type="text" id="bank_clid_zeus" value="<?php if( isset($acting_opts['clientip_bank']) ) echo esc_html($acting_opts['clientip_bank']); ?>" size="40" /></td>
				<td><div id="ex_bank_clid_zeus" class="explanation">契約時にゼウスから発行される入金おまかせサービス用のIPコード（半角数字）</div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_bank_testid_zeus');">テストID</a></th>
				<td colspan="4"><input name="testid_bank" type="text" id="testid_bank_zeus" value="<?php if( isset($acting_opts['testid_bank']) ) echo esc_html($acting_opts['testid_bank']); ?>" size="40" /></td>
				<td><div id="ex_bank_testid_zeus" class="explanation">契約時にゼウスから発行される入金おまかせサービス接続テストで必要なテストID（半角数字）</div></td>
			</tr>
		</table>
		<table class="settle_table">
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_conv_zeus');">コンビニ決済サービス</a></th>
				<td><input name="conv_activate" type="radio" id="conv_activate_zeus_1" value="on"<?php if( isset($acting_opts['conv_activate']) && $acting_opts['conv_activate'] == 'on' ) echo ' checked="checked"'; ?> /></td><td><label for="conv_activate_zeus_1">利用する</label></td>
				<td><input name="conv_activate" type="radio" id="conv_activate_zeus_2" value="off"<?php if( isset($acting_opts['conv_activate']) && $acting_opts['conv_activate'] == 'off' ) echo ' checked="checked"'; ?> /></td><td><label for="conv_activate_zeus_2">利用しない</label></td>
				<td colspan="2"></td>
				<td><div id="ex_conv_zeus" class="explanation">コンビニ支払いができる決済サービスです。払い込みがあった場合、自動的に入金済みになります。</div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_conv_ope_zeus');">稼働環境</a></th>
				<td><input name="conv_ope" type="radio" id="conv_ope_zeus_1" value="test"<?php if( isset($acting_opts['conv_ope']) && $acting_opts['conv_ope'] == 'test' ) echo ' checked="checked"'; ?> /></td><td><label for="conv_ope_zeus_1">テスト環境</label></td>
				<td><input name="conv_ope" type="radio" id="conv_ope_zeus_2" value="public"<?php if( isset($acting_opts['conv_ope']) && $acting_opts['conv_ope'] == 'public' ) echo ' checked="checked"'; ?> /></td><td><label for="conv_ope_zeus_2">本番環境</label></td>
				<td colspan="2"></td>
				<td><div id="ex_conv_ope_zeus" class="explanation">動作環境を切り替えます。</div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_conv_clid_zeus');">コンビニ決済IPコード</a></th>
				<td colspan="6"><input name="clientip_conv" type="text" id="conv_clid_zeus" value="<?php if( isset($acting_opts['clientip_conv']) ) echo esc_html($acting_opts['clientip_conv']); ?>" size="40" /></td>
				<td><div id="ex_conv_clid_zeus" class="explanation">契約時にゼウスから発行されるコンビニ決済サービス用のIPコード（半角数字）</div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_conv_testid_zeus');">テストID</a></th>
				<td colspan="6"><input name="testid_conv" type="text" id="testid_conv_zeus" value="<?php if( isset($acting_opts['testid_conv']) ) echo esc_html($acting_opts['testid_conv']); ?>" size="40" /></td>
				<td><div id="ex_conv_testid_zeus" class="explanation">契約時にゼウスから発行されるコンビニ決済サービス接続テストで必要なテストID（半角数字）</div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_conv_testtype_zeus');">テストタイプ</a></th>
				<td><input name="test_type" type="radio" id="conv_testtype_zeus_1" value="0"<?php if( isset($acting_opts['test_type_conv']) && WCUtils::is_zero($acting_opts['test_type_conv']) ) echo ' checked="checked"'; ?> /></td><td><label for="conv_testtype_zeus_1">入金テスト無し</label></td>
				<td><input name="test_type" type="radio" id="conv_testtype_zeus_2" value="1"<?php if( isset($acting_opts['test_type_conv']) && $acting_opts['test_type_conv'] == 1 ) echo ' checked="checked"'; ?> /></td><td><label for="conv_testtype_zeus_2">売上確定テスト</label></td>
				<td><input name="test_type" type="radio" id="conv_testtype_zeus_3" value="2"<?php if( isset($acting_opts['test_type_conv']) && $acting_opts['test_type_conv'] == 2 ) echo ' checked="checked"'; ?> /></td><td><label for="conv_testtype_zeus_3">売上取消テスト</label></td>
				<td><div id="ex_conv_testtype_zeus" class="explanation">テスト環境でのテストタイプを指定します。</div></td>
			</tr>
			<tr>
				<th rowspan="7"><a style="cursor:pointer;" onclick="toggleVisibility('ex_pay_cvs_zeus');">コンビニ種類</a></th>
				<td><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D001" value="D001"<?php if( isset($acting_opts['pay_cvs']) && in_array( 'D001', $acting_opts['pay_cvs'] ) ) echo ' checked'; ?> /></td><td colspan="5"><label for="pay_cvs_D001"><?php echo esc_html(usces_get_conv_name('D001')); ?></label></td>
				<td rowspan="7"><div id="ex_pay_cvs_zeus" class="explanation">契約時にご利用のお申込みをいただいたコンビニを選択します。</div></td>
			</tr>
			<tr>
				<td><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D002" value="D002"<?php if( isset($acting_opts['pay_cvs']) && in_array( 'D002', $acting_opts['pay_cvs'] ) ) echo ' checked'; ?> /></td><td colspan="5"><label for="pay_cvs_D002"><?php echo esc_html(usces_get_conv_name('D002')); ?></label></td>
			</tr>
			<tr>
				<td><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D030" value="D030"<?php if( isset($acting_opts['pay_cvs']) && in_array( 'D030', $acting_opts['pay_cvs'] ) ) echo ' checked'; ?> /></td><td colspan="5"><label for="pay_cvs_D030"><?php echo esc_html(usces_get_conv_name('D030')); ?></label></td>
			</tr>
			<tr>
				<td><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D040" value="D040"<?php if( isset($acting_opts['pay_cvs']) && in_array( 'D040', $acting_opts['pay_cvs'] ) ) echo ' checked'; ?> /></td><td colspan="5"><label for="pay_cvs_D040"><?php echo esc_html(usces_get_conv_name('D040')); ?></label></td>
			</tr>
			<tr>
				<td><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D015" value="D015"<?php if( isset($acting_opts['pay_cvs']) && in_array( 'D015', $acting_opts['pay_cvs'] ) ) echo ' checked'; ?> /></td><td colspan="5"><label for="pay_cvs_D015"><?php echo esc_html(usces_get_conv_name('D015')); ?></label></td>
			</tr>
			<tr>
				<td><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D050" value="D050"<?php if( isset($acting_opts['pay_cvs']) && in_array( 'D050', $acting_opts['pay_cvs'] ) ) echo ' checked'; ?> /></td><td colspan="5"><label for="pay_cvs_D050"><?php echo esc_html(usces_get_conv_name('D050')); ?></label></td>
			</tr>
			<tr>
				<td><input name="pay_cvs[]" type="checkbox" id="pay_cvs_D060" value="D060"<?php if( isset($acting_opts['pay_cvs']) && in_array( 'D060', $acting_opts['pay_cvs'] ) ) echo ' checked'; ?> /></td><td colspan="5"><label for="pay_cvs_D060"><?php echo esc_html(usces_get_conv_name('D060')); ?></label></td>
			</tr>
		</table>
	...
	<div class="settle_exp">
		...
		<p>銀行振込決済（入金おまかせサービス）及びコンビニ決済のテストを行う際は、「稼働環境」で「テスト環境」を選択し、「テストID」の項目にゼウスから発行されるテストIDを入力してください。<br />
...
<?php
	...
	***********************************************/
	public function acting_transaction() {
		...
		//*** zeus_conv ***//
		} elseif( isset($_REQUEST['acting']) && 'zeus_conv' == $_REQUEST['acting'] && isset($_REQUEST['status']) && isset($_REQUEST['sendpoint']) && isset($_REQUEST['ipcode']) ) {
			foreach( $_REQUEST as $key => $value ) {
				if( 'uscesid' == $key ) {
				} elseif( 'username' == $key ) {
	...
	***********************************************/
	public function set_uscesL10n() {
		...
		if( 'delivery' == $usces->page ) {
			...
			echo "'zeus_form': 'cart',\n";
			echo "'zeus_security': '".$acting_opts['security']."',\n";
			echo "'zeus_quickcharge': '".$acting_opts['quickcharge']."',\n";
			echo "'zeus_howpay': '".$acting_opts['howpay']."',\n";
			echo "'zeus_thisyear': '".date_i18n('Y')."',\n";
			echo "'zeus_pcid': '".$pcid."',\n";
			echo "'zeus_partofcard': '".$partofcard."',\n";
			if( !empty($pcid) && !empty($partofcard) ) {
				...
				echo "'zeus_cardupdate_url': '".urlencode($member_update_settlement)."',\n";
			}

		} elseif( $usces->is_member_page($_SERVER['REQUEST_URI']) ) {
			if( isset($_GET['page'] ) && 'member_update_settlement' == $_GET['page'] ) {
				$acting_opts = $this->get_acting_settings();
				$member = $usces->get_member();
				$pcid = $usces->get_member_meta_value( 'zeus_pcid', $member['ID'] );
				$partofcard = $usces->get_member_meta_value( 'zeus_partofcard', $member['ID'] );
				echo "'zeus_form': 'member',\n";
				echo "'zeus_security': '".$acting_opts['security']."',\n";
				echo "'zeus_quickcharge': '".$acting_opts['quickcharge']."',\n";
				echo "'zeus_thisyear': '".date_i18n('Y')."',\n";
				echo "'zeus_thismonth': '".date_i18n('m')."',\n";
				echo "'zeus_pcid': '".$pcid."',\n";
				echo "'zeus_partofcard': '".$partofcard."',\n";
			}
		}
	...
	public function member_update_settlement_form() {
		...
		if( isset($_POST['zeus_card_update']) ) {
			if( !wp_verify_nonce( $_POST['wc_nonce'], 'member_update_settlement' ) || empty($_POST['zeus_token_value']) ) {
				//wp_redirect( $update_settlement_url );
...
<div class="whitebox">
	...
	<form id="member-card-info" action="<?php echo $update_settlement_url; ?>" method="post" onKeyDown="if(event.keyCode == 13) {return false;}">
		...
		<div class="send">
		<input type="submit" name="zeus_card_update" id="card-update" class="editmember" value="<?php _e('Update'); ?>" />
...
<?php
	...
	***********************************************/
	protected function get_acting_settings() {
		...
		$acting_settings = ( isset($usces->options['acting_settings']['zeus']) ) ? $usces->options['acting_settings']['zeus'] : array();
