...
?>
	...
	<div class="postbox">
		<h3 class="hndle" id="stocklink"><span><?php _e('Stock Linkage OrderData','usces'); ?></span><?php echo $status; ?></h3>
		<div class="inside">
	...
	public function update_order( $new_order, $old_status, $old_order, $new_carts, $old_carts ){
		...
		if( // If the status change was not
			...
			foreach( $old_carts as $ocart ){
				...
				//$newstock = $usces->getItemZaikoNum( $ocart['post_id'], $ocart['sku_code'] );
			}
			...
			foreach( $new_carts as $ncart ){
				...
				//$newstock = $usces->getItemZaikoNum( $ncart['post_id'], $ncart['sku_code'] );
			}
			...
			case 'adminorder':
				if( 
					...
					foreach( $carts as $cart ){
						...
						//$newstock = $usces->getItemZaikoNum( $cart['post_id'], $cart['sku_code'] );
					}
