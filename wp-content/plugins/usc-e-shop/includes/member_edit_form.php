...
<?php
	if( 0 < count($member_metas) ) :
		$cardinfo = array( 'remise'=>array(), 'digitalcheck'=>array(), 'telecom'=>array() );
		foreach( $member_metas as $value ) {
			...
			} elseif( in_array( $value['meta_key'], array( 'telecom_oneclick' ) ) ) {
				$cardinfo['telecom'][$value['meta_key']] = $value['meta_value'];
			}
		}
...
<?php		endif;
		...
		if( 0 < count($cardinfo['telecom']) ) :
			if( array_key_exists( 'telecom_oneclick', $cardinfo['telecom'] ) ) : ?>
		<tr>
			<td class="label">スピード決済</td>
			<td><div class="rod_left shortm"><?php _e('登録あり','usces'); ?></div></td>
		</tr>
		<tr>
			<td class="label"><input type="checkbox" name="telecom_oneclick" value="delete"></td>
			<td>スピード決済を解除する</td>
		</tr>
<?php		endif;
		endif;
	endif;
	do_action( 'usces_action_admin_member_info', $data, $member_metas, $usces_member_history );
?>
</table>
