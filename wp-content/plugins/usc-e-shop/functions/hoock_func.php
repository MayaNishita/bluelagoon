...
//				$log = array( 'acting'=>'remise', 'key'=>$_POST['X-S_TORIHIKI_NO'], 'result'=>'SESSION ERROR', 'data'=>$_POST );
//				usces_save_order_acting_error( $log );
		...
		$_GET['uscesid'] = $datas['sesid'];
		if( empty($datas['sesid']) ) {
			$log = array( 'acting'=>'sbps', 'key'=>$_POST['order_id'], 'result'=>'SESSION ERROR', 'data'=>$_REQUEST );
			usces_save_order_acting_error( $log );
...
//usces_log("digitalcheck conv : ".print_r($_REQUEST,true), 'digitalcheck.log');
	...
	//*** AnotherLane credit ***//
		...
		if( isset($_GET['ali_back']) ) {
			$permalink_structure = get_option( 'permalink_structure' );
			$delim = ( !$usces->use_ssl && $permalink_structure ) ? '?' : '&';
			if( $acting_opts['siteid'] == $_POST['SiteId'] && 'NG' == $_POST['Result'] ) {
				$log = array( 'acting'=>'anotherlane_card', 'key'=>$_POST['TransactionId'], 'result'=>$_POST['Result'], 'data'=>$_POST );
				usces_save_order_acting_error( $log );
				header( 'location: '.USCES_CART_URL.$delim.'confirm=1' );
				exit;
		...
		} elseif( $acting_opts['siteid'] == $_GET['SiteId'] && 'OK' == $_GET['Result'] ) {
			$table_meta_name = $wpdb->prefix."usces_order_meta";
