			...
			for( $i = 0; $i < $opnum; $i++ ) {
				...
				for( $o = 1; $o <= 4; $o++ ) {
					...
					switch( $o ) {
						case 1:
							if( empty($value) ) {
								$oplogtemp .= "No.".$line_num."\t".__(sprintf('Option name of No.%s option is non-input.', ($i+1)), 'usces').$yn;
								$opmestemp .= "No.".$line_num."\t".__(sprintf('Option name of No.%s option is non-input.', ($i+1)), 'usces').$br.$yn;
							}
							$val['name'] = $value;
							break;
						case 2:
							if( $value != NULL && (( 0 > (int)$value) || (5 < (int)$value)) ) {
								$oplogtemp .= "No.".$line_num."\t".__(sprintf('Option-entry-field of No.%s option is abnormal.', ($i+1)), 'usces').$yn;
								$opmestemp .= "No.".$line_num."\t".__(sprintf('Option-entry-field of No.%s option is abnormal.', ($i+1)), 'usces').$br.$yn;
							}
							$val['mean'] = $value;
							break;
						case 3:
							if( $value != NULL && (!preg_match("/^[0-9]+$/", $value) || 1 < (int)$value) ) {
								$oplogtemp .= "No.".$line_num."\t".__(sprintf('Option-required-item of No.%s option is abnormal.', ($i+1)), 'usces').$yn;
								$opmestemp .= "No.".$line_num."\t".__(sprintf('Option-required-item of No.%s option is abnormal.', ($i+1)), 'usces').$br.$yn;
							}
							$val['essential'] = $value;
							break;
						case 4:
							if( ($value != NULL && $value == '') && (2 > $datas[($key-2)] && 0 < strlen($datas[($key-2)])) ) {
								$oplogtemp .= "No.".$line_num."\t".__(sprintf('Option-select of No.%s option is non-input.', ($i+1)), 'usces').$yn;
								$opmestemp .= "No.".$line_num."\t".__(sprintf('Option-select of No.%s option is non-input.', ($i+1)), 'usces').$br.$yn;
							}
							$val['value'] = $value;
							break;
					}
...
function usces_download_item_list() {
	...
	if( !empty($results) ) {
		...
		foreach( (array)$rows as $row ) {
			...
			$line_item = apply_filters( 'usces_filter_downloadcsv_add_itemvalue', $line_item, $post_id );
...
//if( !function_exists('usces_item_stock_uploadcsv') ):
...
// item stock list download
//if( !function_exists('usces_download_item_stock_list') ):
